...
int aes_xr_test()
{
	WORD key_schedule[120], std_key_schedule[60];
	BYTE enc_buf[128];
	...
	BYTE std_ciphertext[16], std_decrypted[16];
	...
	pass = pass && !memcmp(plaintext, decrypted, 16);
	...
	pass = pass && !memcmp(test_plain, test_decrypt, 16);
	...
	printf(" (\"%s\")\n", test_decrypt);
	printf("  Test result:  %s\n", memcmp(test_plain, test_decrypt, 16) == 0 ? "PASS" : "FAIL");

	// Comparison with standard AES
	aes_key_setup(key, std_key_schedule, 128);
	aes_encrypt(plaintext, std_ciphertext, std_key_schedule, 128);
	aes_decrypt(std_ciphertext, std_decrypted, std_key_schedule, 128);

	printf("\n* AES vs AES-XR Comparison:\n");
	printf("  Same plaintext:  ");
	print_hex(plaintext, 16);
	printf("\n  Standard AES:    ");
	print_hex(std_ciphertext, 16);
	printf("\n  AES-XR:          ");
	print_hex(ciphertext, 16);
	printf("\n  Different:       %s\n", memcmp(std_ciphertext, ciphertext, 16) != 0 ? "YES (as expected)" : "NO");
	printf("  Std round-trip:  %s\n", memcmp(plaintext, std_decrypted, 16) == 0 ? "PASS" : "FAIL");
