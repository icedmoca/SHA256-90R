...
// AES-XR (Extended Rounds) definitions
#define AES_XR_128_ROUNDS 20
#define AES_XR_192_ROUNDS 24
#define AES_XR_256_ROUNDS 28
...
// AES-XR Extended S-boxes - Generated using different mathematical properties
// These provide enhanced security through extended rounds
static const BYTE aes_xr_sbox[16][16] = {
...
static const BYTE aes_xr_invsbox[16][16] = {
	{0x63,0x7C,0x77,0x7B,0xF2,0x6B,0x6F,0xC5,0x30,0x01,0x67,0x2B,0xFE,0xD7,0xAB,0x76},
	{0xCA,0x82,0xC9,0x7D,0xFA,0x59,0x47,0xF0,0xAD,0xD4,0xA2,0xAF,0x9C,0xA4,0x72,0xC0},
	{0xB7,0xFD,0x93,0x26,0x36,0x3F,0xF7,0xCC,0x34,0xA5,0xE5,0xF1,0x71,0xD8,0x31,0x15},
	{0x04,0xC7,0x23,0xC3,0x18,0x96,0x05,0x9A,0x07,0x12,0x80,0xE2,0xEB,0x27,0xB2,0x75},
...
/*********************** AES-XR FUNCTION DEFINITIONS ***********************/
// AES-XR SubWord function using extended S-box
WORD SubWord_XR(WORD word)
{
	unsigned int result;

	result = (int)aes_xr_sbox[(word >> 4) & 0x0000000F][word & 0x0000000F];
	result += (int)aes_xr_sbox[(word >> 12) & 0x0000000F][(word >> 8) & 0x0000000F] << 8;
	result += (int)aes_xr_sbox[(word >> 20) & 0x0000000F][(word >> 16) & 0x0000000F] << 16;
	result += (int)aes_xr_sbox[(word >> 28) & 0x0000000F][(word >> 24) & 0x0000000F] << 24;
	return(result);
}

// AES-XR Key setup with extended rounds
void aes_xr_key_setup(const BYTE key[], WORD w[], int keysize)
{
	int Nb=4,Nr,Nk,idx;
	WORD temp,Rcon[]={0x01000000,0x02000000,0x04000000,0x08000000,0x10000000,0x20000000,
	                  0x40000000,0x80000000,0x1b000000,0x36000000,0x6c000000,0xd8000000,
	...
	switch (keysize) {
		case 128: Nr = AES_XR_128_ROUNDS; Nk = 4; break;
		case 192: Nr = AES_XR_192_ROUNDS; Nk = 6; break;
		case 256: Nr = AES_XR_256_ROUNDS; Nk = 8; break;
		default: return;
	}
	...
	for (idx = Nk; idx < Nb * (Nr+1); ++idx) {
		temp = w[idx - 1];
		if ((idx % Nk) == 0)
			temp = SubWord_XR(KE_ROTWORD(temp)) ^ Rcon[(idx-1)/Nk];
		else if (Nk > 6 && (idx % Nk) == 4)
			temp = SubWord_XR(temp);
		w[idx] = w[idx-Nk] ^ temp;
	}
}

// AES-XR SubBytes using extended S-box
void SubBytes_XR(BYTE state[][4])
{
	state[0][0] = aes_xr_sbox[state[0][0] >> 4][state[0][0] & 0x0F];
	state[0][1] = aes_xr_sbox[state[0][1] >> 4][state[0][1] & 0x0F];
	state[0][2] = aes_xr_sbox[state[0][2] >> 4][state[0][2] & 0x0F];
	state[0][3] = aes_xr_sbox[state[0][3] >> 4][state[0][3] & 0x0F];
	state[1][0] = aes_xr_sbox[state[1][0] >> 4][state[1][0] & 0x0F];
	state[1][1] = aes_xr_sbox[state[1][1] >> 4][state[1][1] & 0x0F];
	state[1][2] = aes_xr_sbox[state[1][2] >> 4][state[1][2] & 0x0F];
	state[1][3] = aes_xr_sbox[state[1][3] >> 4][state[1][3] & 0x0F];
	state[2][0] = aes_xr_sbox[state[2][0] >> 4][state[2][0] & 0x0F];
	state[2][1] = aes_xr_sbox[state[2][1] >> 4][state[2][1] & 0x0F];
	state[2][2] = aes_xr_sbox[state[2][2] >> 4][state[2][2] & 0x0F];
	state[2][3] = aes_xr_sbox[state[2][3] >> 4][state[2][3] & 0x0F];
	state[3][0] = aes_xr_sbox[state[3][0] >> 4][state[3][0] & 0x0F];
	state[3][1] = aes_xr_sbox[state[3][1] >> 4][state[3][1] & 0x0F];
	state[3][2] = aes_xr_sbox[state[3][2] >> 4][state[3][2] & 0x0F];
	state[3][3] = aes_xr_sbox[state[3][3] >> 4][state[3][3] & 0x0F];
}

void InvSubBytes_XR(BYTE state[][4])
{
	state[0][0] = aes_xr_invsbox[state[0][0] >> 4][state[0][0] & 0x0F];
	state[0][1] = aes_xr_invsbox[state[0][1] >> 4][state[0][1] & 0x0F];
	state[0][2] = aes_xr_invsbox[state[0][2] >> 4][state[0][2] & 0x0F];
	state[0][3] = aes_xr_invsbox[state[0][3] >> 4][state[0][3] & 0x0F];
	state[1][0] = aes_xr_invsbox[state[1][0] >> 4][state[1][0] & 0x0F];
	state[1][1] = aes_xr_invsbox[state[1][1] >> 4][state[1][1] & 0x0F];
	state[1][2] = aes_xr_invsbox[state[1][2] >> 4][state[1][2] & 0x0F];
	state[1][3] = aes_xr_invsbox[state[1][3] >> 4][state[1][3] & 0x0F];
	state[2][0] = aes_xr_invsbox[state[2][0] >> 4][state[2][0] & 0x0F];
	state[2][1] = aes_xr_invsbox[state[2][1] >> 4][state[2][1] & 0x0F];
	state[2][2] = aes_xr_invsbox[state[2][2] >> 4][state[2][2] & 0x0F];
	state[2][3] = aes_xr_invsbox[state[2][3] >> 4][state[2][3] & 0x0F];
	state[3][0] = aes_xr_invsbox[state[3][0] >> 4][state[3][0] & 0x0F];
	state[3][1] = aes_xr_invsbox[state[3][1] >> 4][state[3][1] & 0x0F];
	state[3][2] = aes_xr_invsbox[state[3][2] >> 4][state[3][2] & 0x0F];
	state[3][3] = aes_xr_invsbox[state[3][3] >> 4][state[3][3] & 0x0F];
}

// AES-XR Encryption with extended rounds
void aes_xr_encrypt(const BYTE in[], BYTE out[], const WORD key[], int keysize)
{
	BYTE state[4][4];
	int round, max_rounds;

	// Copy input to state
	state[0][0] = in[0]; state[1][0] = in[1]; state[2][0] = in[2]; state[3][0] = in[3];
	state[0][1] = in[4]; state[1][1] = in[5]; state[2][1] = in[6]; state[3][1] = in[7];
	state[0][2] = in[8]; state[1][2] = in[9]; state[2][2] = in[10]; state[3][2] = in[11];
	state[0][3] = in[12]; state[1][3] = in[13]; state[2][3] = in[14]; state[3][3] = in[15];

	// Determine number of rounds
	switch (keysize) {
		case 128: max_rounds = AES_XR_128_ROUNDS; break;
		case 192: max_rounds = AES_XR_192_ROUNDS; break;
		case 256: max_rounds = AES_XR_256_ROUNDS; break;
		default: return;
	}
	...
	SubBytes_XR(state);
	ShiftRows(state);
	AddRoundKey(state, &key[max_rounds * 4]);

	// Copy state to output
	out[0] = state[0][0]; out[1] = state[1][0]; out[2] = state[2][0]; out[3] = state[3][0];
	out[4] = state[0][1]; out[5] = state[1][1]; out[6] = state[2][1]; out[7] = state[3][1];
	out[8] = state[0][2]; out[9] = state[1][2]; out[10] = state[2][2]; out[11] = state[3][2];
	out[12] = state[0][3]; out[13] = state[1][3]; out[14] = state[2][3]; out[15] = state[3][3];
}

// AES-XR Decryption with extended rounds
void aes_xr_decrypt(const BYTE in[], BYTE out[], const WORD key[], int keysize)
{
	BYTE state[4][4];
	int round, max_rounds;
	...
	// Main rounds - InvShiftRows, InvSubBytes, AddRoundKey, InvMixColumns (for rounds max_rounds-1 downto 1)
	for (round = max_rounds - 1; round > 0; round--) {
		InvShiftRows(state);
		InvSubBytes_XR(state);
		AddRoundKey(state, &key[round * 4]);
		InvMixColumns(state);
	}
