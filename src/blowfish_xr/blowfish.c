   0x68DC1462,0xD7486900,0x680EC0A4,0x27A18DEE,0x4F3FFEA2,0xE887AD8C,0xB58CE006,0x7AF4D6B6,
   ...
   0x4A39CE37,0xD3FAF5CF,0xABC27737,0x5AC52D1B,0x5CB0679E,0x4FA33742,0xD3822740,0x99BC9BBE,
   0xD5118E9D,0xBF0F7315,0xD62D1C7E,0xC700C47B,0xB78C1B6B,0x21A19045,0xB26EB1BE,0x6A366EB4,
   0x5748AB2F,0xBC946E79,0xC6A376D2,0x6549C2C8,0x530FF8EE,0x468DDE7D,0xD5730A1D,0x4CD04DC6,
   0x2939BBDB,0xA9BA4650,0xAC9526E8,0xBE5EE304,0xA1FAD5F0,0x6A2D519A,0x63EF8CE2,0x9A86EE22,
   0xC089C2B8,0x43242EF6,0xA51E03AA,0x9CF2D0A4,0x83C061BA,0x9BE96A4D,0x8FE51550,0xBA645BD6,
...
/*********************** BLOWFISH-XR FUNCTION DEFINITIONS ***********************/
// Blowfish-XR with 32 rounds and extended S-boxes
void blowfish_xr_encrypt(const BYTE in[], BYTE out[], const BLOWFISH_XR_KEY *keystruct)
{
   ...
   // 32 rounds for Blowfish-XR
   for (i = 0; i < 32; i++) {
      l ^= keystruct->p[i];
      F(l,t); r^= t; swap(r,l,t);
   }
...
void blowfish_xr_decrypt(const BYTE in[], BYTE out[], const BLOWFISH_XR_KEY *keystruct)
{
   ...
   // Exact reverse of Blowfish-XR encryption

   // Undo final operations
   r ^= keystruct->p[33];      // Undo R ^= P[33]
   F(l, t); r ^= t;            // Undo R ^= F(L)
   l ^= keystruct->p[32];      // Undo L ^= P[32]

   // Undo the 32 rounds in reverse order
   for (i = 31; i >= 0; i--) {
      swap(r,l,t);             // Undo swap(L,R)
      F(l, t);                 // Compute F(L) for undoing R ^= F(L)
      r ^= t;                  // Undo R ^= F(L)
      l ^= keystruct->p[i];    // Undo L ^= P[i]
   }
...
void blowfish_xr_key_setup(const BYTE user_key[], BLOWFISH_XR_KEY *keystruct, size_t len)
{
   ...
   // Copy over the constant init array vals (so the originals aren't destroyed).
   memcpy(keystruct->p,p_perm_xr,sizeof(WORD) * 34);
   memcpy(keystruct->s,s_perm_xr,sizeof(WORD) * 1024);

   // Combine the key with the P box. Assume key is standard 448 bits (56 bytes) or less.
   for (idx = 0, idx2 = 0; idx < 34; ++idx, idx2 += 4)
      keystruct->p[idx] ^= (user_key[idx2 % len] << 24) | (user_key[(idx2+1) % len] << 16)
                           | (user_key[(idx2+2) % len] << 8) | (user_key[(idx2+3) % len]);
   ...
   for (idx = 0; idx < 34; idx += 2) {
      blowfish_xr_encrypt(block,block,keystruct);
      keystruct->p[idx] = (block[0] << 24) | (block[1] << 16) | (block[2] << 8) | block[3];
      keystruct->p[idx+1]=(block[4] << 24) | (block[5] << 16) | (block[6] << 8) | block[7];
   }
