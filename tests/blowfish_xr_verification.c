/*********************************************************************
* Filename:   blowfish_xr_verification.c
* Author:     Blowfish-XR Verification Test Suite
* Copyright:
* Disclaimer: This code is presented "as is" without any guarantees.
* Details:    Comprehensive verification test for Blowfish-XR including
...
#include <math.h>
#include <stdint.h>
#include "../src/blowfish_xr/blowfish.h"
...
/**************************** GLOBAL VARIABLES ****************************/
// Test vectors for Blowfish-XR verification
BYTE test_key[] = "MySecretKey";
BYTE test_plaintext[TEST_BLOCK_SIZE] = {0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0};
...
/**
 * Time a single Blowfish-XR operation
 */
double time_blowfish_xr_encrypt(const BYTE *plaintext, const BYTE *key, size_t key_len, BYTE *ciphertext) {
    struct timespec start, end;
    BLOWFISH_XR_KEY key_struct;

    // Setup key (not timed)
    blowfish_xr_key_setup(key, &key_struct, key_len);

    // Start timing
    clock_gettime(CLOCK_MONOTONIC_RAW, &start);

    // Encrypt
    blowfish_xr_encrypt(plaintext, ciphertext, &key_struct);
 ...
 */
void collect_timing_samples(double *samples, size_t count, const BYTE *input, const BYTE *key, size_t key_len) {
    printf("Collecting %zu timing samples...\n", count);

    for (size_t i = 0; i < count; i++) {
        BYTE ciphertext[TEST_BLOCK_SIZE];
        samples[i] = time_blowfish_xr_encrypt(input, key, key_len, ciphertext);
 ...
 */
int test_blowfish_xr_correctness() {
    printf("=== Blowfish-XR Functional Correctness Test ===\n");
    ...
    // Setup key
    blowfish_xr_key_setup(test_key, &key_struct, strlen((char*)test_key));

    // Encrypt
    blowfish_xr_encrypt(test_plaintext, ciphertext, &key_struct);

    // Decrypt
    blowfish_xr_decrypt(ciphertext, decrypted, &key_struct);

    print_hex(test_plaintext, TEST_BLOCK_SIZE, "Original Plaintext");
    print_hex(ciphertext, TEST_BLOCK_SIZE, "Blowfish-XR Ciphertext");
 ...
 */
void benchmark_blowfish_xr() {
    printf("\n=== Blowfish-XR Performance Benchmark ===\n");
...
void test_timing_side_channels() {
    printf("\n=== Blowfish-XR Timing Side-Channel Analysis ===\n");
    ...
    collect_timing_samples(samples1, NUM_SAMPLES, input1, test_key, strlen((char*)test_key));

    printf("Input 2: Single bit flip\n");
    collect_timing_samples(samples2, NUM_SAMPLES, input2, test_key, strlen((char*)test_key));
...
void test_edge_cases() {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        blowfish_xr_encrypt(test_cases[i], ciphertext, &key_struct);
        blowfish_xr_decrypt(ciphertext, decrypted, &key_struct);
...
void test_known_vectors() {
    printf("\n=== Blowfish-XR Known Test Vectors ===\n");
    ...
    BYTE input1[TEST_BLOCK_SIZE] = {'t', 'e', 's', 't', 'd', 'a', 't', 'a'};
    blowfish_xr_encrypt(input1, ciphertext, &key_struct);
    print_hex(input1, TEST_BLOCK_SIZE, "Input 'testdata'");
    print_hex(ciphertext, TEST_BLOCK_SIZE, "Blowfish-XR output");

    // Test vector 2: Empty string (all zeros)
    BYTE input2[TEST_BLOCK_SIZE] = {0};
    blowfish_xr_encrypt(input2, ciphertext, &key_struct);
    print_hex(input2, TEST_BLOCK_SIZE, "Input empty string");
    print_hex(ciphertext, TEST_BLOCK_SIZE, "Blowfish-XR output");

    // Test vector 3: "foobar" (padded)
    BYTE input3[TEST_BLOCK_SIZE] = {'f', 'o', 'o', 'b', 'a', 'r', 0, 0};
    blowfish_xr_encrypt(input3, ciphertext, &key_struct);
...
/*********************** MAIN FUNCTION ***********************/
int main() {
    printf("=== Blowfish-XR Comprehensive Verification Test Suite ===\n");
    printf("Testing functional correctness, performance, and security\n\n");

    // Run all tests
    int functional_correct = test_blowfish_xr_correctness();
    benchmark_blowfish_xr();
    test_timing_side_channels();
    test_edge_cases();
    test_known_vectors();

    // Summary
    printf("\n=== Blowfish-XR Verification Summary ===\n");
    ...
    printf("\nBlowfish-XR verification completed successfully!\n");
